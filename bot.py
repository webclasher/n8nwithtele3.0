...
def api_headers():
    # Add support for N8N_API_KEY if set
...
def run_workflow(wf_id):
    try:
        # Endpoint for execution may differ by n8n version; try commonly used endpoints
    ...
    try:
        if data == "backup_n8n":
            ...
            try:
                ...
                await query.edit_message_text(f"Delete logs failed: {e}")
            ...
            try:
                ...
                await query.edit_message_text(f"Delete backups failed: {e}")
...
def main():
    ...
    app.add_handler(CommandHandler("list_workflows", lambda u,c: u.message.reply_text('Use /start and the List Workflows button or press List Workflows from the main menu.')))

    # Callback queries (inline buttons)
    app.add_handler(CallbackQueryHandler(callback_handler))

    # File handler for restores (both n8n backups and workflow json)
